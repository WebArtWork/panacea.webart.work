# PANACEA storefront (Angular 22, SSR + Prerender)

Ukrainian marketplace-style storefront for PANACEA mineral water (Arden and Diamond lines): a
catalog with filtering, product detail pages, cart, checkout, order confirmation, informational
pages, and a static demo admin/CRM section. Built on the `@wawjs` Angular landing template with
**SSR prerendering**, **TailwindCSS**, and **GitHub Pages deployment**.

There is no backend: the catalog, cart, checkout, and admin section all run on local browser
storage and local fallback data under `src/data/`. Checkout, payments, and real order/customer
persistence are not implemented — the checkout page and cart explain this and link to the company
phone number.

This README documents the underlying Angular template. For what's specific to PANACEA, see
`src/app/feature/`, `src/app/pages/`, `src/data/`, and the storefront/admin styles under
`src/styles/`.
